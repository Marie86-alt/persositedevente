...
<div class="container mx-auto">
    ...
    <form action="{{ route('orders.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="customer_name" id="customer_name" class="shadow appearance-none border rounded w-1/2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>

        <div class="mb-4">
            <label for="order_date" class="block text-gray-700 text-sm font-bold mb-2">Date de la Commande :</label>
            <input type="date" name="order_date" id="order_date" class="shadow appearance-none border rounded w-1/2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>

        <div class="mb-4">
            <label for="total_amount" class="block text-gray-700 text-sm font-bold mb-2">Montant Total :</label>
            <input type="number" step="0.01" name="total_amount" id="total_amount" class="shadow appearance-none border rounded w-1/2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>
